...
import { ForceUpdateNotifcationsRequest, MessageKinds, NotificationsChecked } from 'src/service-worker/messages';
...
export class NotificationService {
            ...
            const resolvePromise = (ev: MessageEvent) => {
                ...
                switch (message.type) {
                    case "notifications":
                        ...
                        resolve()
                        navigator.serviceWorker.removeEventListener('message', resolvePromise)
                }
            }

            navigator.serviceWorker.addEventListener('message', resolvePromise)
        })
    }
    ...
    updateNotifications(notifications: Notifications) {
        ...
        const oldNotifs = JSON.stringify(this.notifications && this.notifications.items)
