...
export class RantDetailPage implements OnInit, OnDestroy {
  ...
  submitComment() {
    ...
    if (this.commentString.length < 1) {
      ...
      this.alertService.showAlert('Comment too Long', 'Comment length cannot exceed 1000 characters');
      return;
    }

    if (this.service.isSignedIn) {
      const token = this.service.token;
      const response = this.service.postComment(this.rant.id, this.commentString, token);
