...
md.renderer.rules.link_open = function (tokens, idx, options, env, self) {
  ...
  attrs.forEach(attr => {
    ...
    if (!href.startsWith('/')) {
      ...
      tokens[idx].attrPush(['onclick', `window.dispatchEvent(new CustomEvent('internal-link', { detail: '${href}' }))`]);
...
export class RenderContentComponent implements OnInit {
  ...
  async linkMentionAsync(name) {
    ...
    this.internalContent = this.internalContent.replace(`@${name}`, `[${name}](/user/${userId})`)
  }
  ...
  renderContent() {
    ...
    (this.links || []).filter(link => {
      ...
      markdown = markdown.replace(link.short_url, `[${link.title}](${link.url})`)
    })
